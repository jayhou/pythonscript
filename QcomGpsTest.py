...
def prepareTest(reportFile):
    ...
    subprocess.call("adb root && adb shell settings put global stay_on_while_plugged_in 3")
...
def startTestSportGps():
    ...
    with open(filename, 'w') as reportFile:
        ...
        while failed is False:
            ...
            p = subprocess.Popen("adb wait-for-devices && adb logcat", shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
